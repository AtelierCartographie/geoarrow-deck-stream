 ...
 */
function ensureNativeGeoArrow(table: Table): {
  table: Table;
  geomType: ReturnType<typeof detectGeometryType>;
} {
...
function runParse(
  ...
  if (geomType === "unknown" || geomType === "wkb") {
    throw new Error(
      "Unsupported geometry type or missing ARROW:extension:name metadata",
    );
  }
  ...
  const sink = createBinarySink({
    initialCoordCapacity: Math.ceil(
      (sizes.positionCapacity * capacityMultiplier) / 2,
    ),
  ...
  return {
    data: outputData,
    debug: collectDebug ? sink.getDebugInfo() : undefined,
  };
}
...
function streamMultiPolygons(
  ...
  if (separatedCoords) {
    ...
    for (let featureIdx = 0; featureIdx < length; featureIdx++) {
      ...
      for (
        let polygonIdx = polygonStart;
        polygonIdx < polygonEnd;
        polygonIdx++
      ) {
 ...
 */
export function createIdentityParser(
  options: {
    reflectY?: boolean;
    scale?: number;
  ...
  return (input: GeometryInput) => parseGeometry(input, { projection });
}
...
export function parsePoints(
  ...
  const { table, geomType } = ensureNativeGeoArrow(inputTable);
  const projectPoint = createPointProjector(projection);

  if (geomType !== "point" && geomType !== "multipoint") {
    throw new Error(
      `parsePoints requires Point or MultiPoint geometry, got: ${geomType}`,
    );
  }
  ...
  const positions = new Float32Array(
    Math.ceil(coordCount * 2 * capacityMultiplier),
  );
  const featureIds = new Uint32Array(
    Math.ceil(coordCount * capacityMultiplier),
  );
...
export function parsePolygonsToSolid(
  ...
  if (geomType !== "polygon" && geomType !== "multipolygon") {
    throw new Error(
      `parsePolygonsToSolid requires Polygon or MultiPolygon geometry, got: ${geomType}`,
    );
  }
 ...
 */
export type GeometryTypeString =
  | "point"
  | "multipoint"
  | "linestring"
  | "polygon"
