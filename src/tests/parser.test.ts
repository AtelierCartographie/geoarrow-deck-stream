...
function getGeometryVector(table: Table): Vector {
  ...
  if (!geomCol) {
    throw new Error(
      `No geometry column found in table. Columns: ${table.schema.fields.map((f) => f.name).join(", ")}`,
    );
  }
  return geomCol;
}
...
describe("parseGeometry - Polygon", () => {
  ...
  beforeAll(() => {
    ...
    polygonWithHolesTable = loadArrowTable(
      "polygons-with-holes.interleaved.arrow",
    );
  });
...
describe("Deck.gl integration", () => {
  ...
  it("should create SolidPolygonLayer props with correct indices", () => {
    ...
    expect(props.data.attributes.vertexValid).toBeUndefined();
  ...
  it("should create SolidPolygonLayer props with holes (triangulated)", () => {
    ...
    expect(props.data.attributes.vertexValid).toBeDefined();
    const indices = props.data.attributes.indices;
    const vertexValid = props.data.attributes.vertexValid;

    expect(indices!.length).toBeGreaterThan(0);
    expect(indices!.length % 3).toBe(0);
    expect(vertexValid?.size).toBe(1);
    expect(
      Array.from((vertexValid!.value as Uint8Array).values()).filter(
        (value) => value === 0,
      ).length,
    ).toBe(polygonData.holeIndices.length - 1);
  });
...
describe("parsePoints", () => {
  let pointsTable: Table;

  beforeAll(() => {
    pointsTable = loadArrowTable("points.interleaved.arrow");
  });

  it("should project points through the projection stream", () => {
    const identity = parsePoints(pointsTable, {
      projection: geoIdentity(),
    });
    ...
    } as unknown as Parameters<typeof parsePoints>[1]["projection"];

    const streamed = parsePoints(pointsTable, {
      projection: translatedProjection,
    });

    expect(streamed.length).toBe(identity.length);
    expect(streamed.positions[0]).toBeCloseTo(identity.positions[0] + 10);
    expect(streamed.positions[1]).toBeCloseTo(identity.positions[1] - 5);
  });
});
